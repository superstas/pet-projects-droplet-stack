...
setup() {
    ...
    eval "$(sed -n '/^sanitize_domain()/,/^}/p' "$PROJECT_ROOT/infra/scripts/setup-application.sh")"
}
...
@test "nginx config: contains proxy_pass directive" {
    ...
    run grep -q 'proxy_pass http://localhost:$APP_PORT' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "nginx config: contains SSL configuration" {
    ...
    run grep -q "setup-ssl.sh" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "nginx config: contains static files location" {
    # Verify the script configures static files
    run grep -q "location /static" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "nginx config: contains www to non-www redirect" {
    # Verify the script creates www redirect
    run grep -q "www.$DOMAIN_NAME" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "nginx config: contains HTTP to HTTPS redirect" {
    ...
    run grep -q "listen 80;" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "systemd service: generated service contains [Unit] section" {
    ...
    run grep -A 2 "cat > \"\$SERVICE_FILE\"" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
    
    run grep -q "\[Unit\]" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: contains [Service] section" {
    run grep -q "\[Service\]" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: contains [Install] section" {
    run grep -q "\[Install\]" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: sets User to system username" {
    run grep -q "User=\$USERNAME" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: sets WorkingDirectory" {
    run grep -q "WorkingDirectory=\$USER_HOME/app" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: configures Restart=always" {
    run grep -q "Restart=always" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: configures StandardOutput=journal" {
    run grep -q "StandardOutput=journal" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "systemd service: sets WantedBy=multi-user.target" {
    run grep -q "WantedBy=multi-user.target" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "prometheus config: script updates prometheus.yml" {
    run grep -q "PROMETHEUS_CONFIG=\"/etc/prometheus/prometheus.yml\"" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: creates backup before modification" {
    run grep -q "PROMETHEUS_BACKUP=" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: adds job_name for application" {
    run grep -q "job_name: '\$USERNAME'" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: configures metrics_path" {
    run grep -q "metrics_path: '\$METRICS_PATH'" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: adds static_configs with target" {
    run grep -q "targets: \['localhost:\$APP_PORT'\]" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: validates configuration with promtool" {
    run grep -q "promtool check config" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "prometheus config: reloads prometheus after update" {
    run grep -q "systemctl reload prometheus" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "error handling: script uses set -e" {
    run head -n 5 "$PROJECT_ROOT/infra/scripts/setup-application.sh"
...
@test "error handling: validates number of arguments" {
    run grep -q 'if \[ "\$#" -ne 4 \]' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: validates domain name not empty" {
    run grep -q 'if \[ -z "\$DOMAIN_NAME" \]' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: validates port number range" {
    # Check that the script validates port is numeric and in valid range
    run grep -q 'APP_PORT.*=~.*0-9' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
    run grep -q 'APP_PORT.*-lt 1024' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
    run grep -q 'APP_PORT.*-gt 65535' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: validates metrics path not empty" {
    run grep -q 'if \[ -z "\$METRICS_PATH" \]' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: validates SSH public key not empty" {
    run grep -q 'if \[ -z "\$SSH_PUBLIC_KEY" \]' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: validates generated username not empty" {
    run grep -q 'if \[ -z "\$USERNAME" \]' "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "error handling: checks nginx configuration validity" {
    run grep -q "nginx -t" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
...
@test "error handling: exits on certbot failure" {
    ...
    run grep -A 10 "nginx -t" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
...
@test "logging: script defines log_info function" {
    run grep -q "log_info()" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "logging: script defines log_warn function" {
    run grep -q "log_warn()" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "logging: script defines log_error function" {
    run grep -q "log_error()" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}

@test "logging: uses color codes for output" {
    run grep -q "GREEN=" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
    run grep -q "RED=" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
    run grep -q "YELLOW=" "$PROJECT_ROOT/infra/scripts/setup-application.sh"
    assert_success
}
