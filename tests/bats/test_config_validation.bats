...
@test "cloud-init.yml exists" {
    [ -f "$PROJECT_ROOT/templates/cloud-init.yml" ]
}

@test "cloud-init.yml is valid YAML" {
    run python3 -c "import yaml; yaml.safe_load(open('$PROJECT_ROOT/templates/cloud-init.yml'))"
    assert_success
}
...
@test "cloud-init.yml contains required packages" {
    ...
    for package in "${required_packages[@]}"; do
        run grep -q "$package" "$PROJECT_ROOT/templates/cloud-init.yml"
        assert_success
    done
}
...
@test "cloud-init.yml disables password authentication" {
    run grep -q "PasswordAuthentication no" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}

@test "cloud-init.yml enables pubkey authentication" {
    run grep -q "PubkeyAuthentication yes" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}
...
@test "cloud-init.yml configures UFW firewall" {
    ...
    assert_success
    run grep -q "ufw allow 443/tcp" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}

@test "cloud-init.yml creates 1GB swap" {
    run grep -q "bs=1M count=1024" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}

@test "cloud-init.yml sets swappiness to 10" {
    run grep -q "vm.swappiness=10" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}

@test "cloud-init.yml configures log retention to 90 days" {
    run grep -q "MaxRetentionSec=90d" "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}

@test "cloud-init.yml disables automatic reboot" {
    run grep -q 'Automatic-Reboot "false"' "$PROJECT_ROOT/templates/cloud-init.yml"
    assert_success
}
...
@test "setup-application.sh exists" {
    [ -f "$PROJECT_ROOT/scripts/setup-application.sh" ]
}

@test "setup-application.sh is executable" {
    [ -x "$PROJECT_ROOT/scripts/setup-application.sh" ]
}

@test "setup-application.sh has shebang" {
    run head -n 1 "$PROJECT_ROOT/scripts/setup-application.sh"
    assert_output --partial "#!/bin/bash"
}

@test "setup-application.sh uses set -e for error handling" {
    run grep -q "set -e" "$PROJECT_ROOT/scripts/setup-application.sh"
    assert_success
}

@test "setup-application.sh defines sanitize_domain function" {
    run grep -q "sanitize_domain()" "$PROJECT_ROOT/scripts/setup-application.sh"
    assert_success
}
